...
./onefs-node-enroll.sh uefi $ONEFS_NODE
...
MGMT_PORT=$(openstack baremetal port list --node $NODE_UUID --field uuid --field address --field local_link_connection | grep frontend | grep -v -i ${PXE_PORT1[0]} | grep -v trunk | awk '{print $2}')
...
DATA_PORT=$(openstack baremetal port list --node $NODE_UUID --field uuid --field address --field local_link_connection | grep frontend | grep -v -i ${PXE_PORT1[0]} | grep trunk | awk '{print $2}')

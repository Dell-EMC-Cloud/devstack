...
baremetal port create ${PXE_PORT2[0]} --node $NODE_UUID --pxe-enabled true --physical-network fsf-net --local-link-connection switch_id=11:22:33:44:55:66 --local-link-connection port_id="${PXE_PORT2[2]}" --local-link-connection switch_info="{'switch_ip': '${PXE_PORT2[1]}', 'cluster': 'TestCustomer1', 'preemption': false, 'access_mode': 'access', 'fabric': 'frontend'}"
